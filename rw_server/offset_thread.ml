...
(*    Mutex.lock Files_progress.mutex_ht ;*)
    ...
    Hashtbl.iter (fun (wd, file) (date, filesize, (isfirstoffsetknown, _, error_counter), sql_obj_opt, created) ->
      let offset_opt =
	...
	| None ->
	  ...
	  if error_counter' < 2 then begin
	    Hashtbl.replace Files_progress.ht
	      (wd, file)
	      (date, filesize, (isfirstoffsetknown, offset_opt, error_counter'), sql_obj_opt, created);
	...
	| Some _ ->
	  ...
	  Hashtbl.replace Files_progress.ht
	    (wd, file)
	    (date, filesize, (true, offset_opt, 0), sql_obj_opt, created) ;
